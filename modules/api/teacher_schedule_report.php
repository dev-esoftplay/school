...
foreach ($schedules as $schedule) {
  ...
  $date   = $days . '|' . $date . '|'. $date_day;
  $report = $months . '|' . $weeks;

  $schedule_report[$report][$date][]   = $item;
}
...
foreach ($schedule_report as $report => $schedule_days) {
  [$months, $weeks] = explode('|', $report);

  $result = (!empty($week) && empty($month) ? ['week' => $weeks] :
            (empty($week) && empty($month) && empty($day) ? ['week' => $weeks] :
            (!empty($week) && !empty($month) ? ['week' => $weeks] :
            (!empty($month) ? ['month' => $months] : ['week' => $weeks]))));

  $result['schedule_days'] = [];

  foreach ($schedule_days as $date => $schedules) {
    [$day, $date, $date_day] = explode('|', $date);
    $result_day = array(
      'day'               => $day,
      'date'              => $date,
      'date_day'          => $date_day,
