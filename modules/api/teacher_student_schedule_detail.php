...
$query = $db->getall('SELECT sa.`schedule_id`, sa.`presence`, st.`name` as `teacher`, sc.`name` as subject, ss.`clock_start`, ss.`clock_end`
          ...
          WHERE sa.`student_id` = '. $student_id .' AND DATE(sa.`created`) = "' . $formatted_date . '"');
