...
$tanggal  			= $_GET['date']; 
$formatted_date = date('Y-m-d', strtotime($tanggal));
...
if ($schedule_class) 
{
	...
	{
		$notes_result = $db->getrow('SELECT `presence`, `notes` FROM `school_attendance` WHERE `schedule_id`= '. $schedule_id .' AND `student_id` = '. $student['student_id'] .' AND DATE(created) = "'. $formatted_date .'"');
