...
foreach ($schedule_ids as $schedule_id) {
  foreach ($data as $entry) {
    ...
    $parent_user_id = $db->getrow('SELECT `parent_id_dad`, `parent_id_mom`, `parent_id_guard` FROM `school_student` WHERE `id` = ' . $entry['student_id']);
    switch ($entry['status']) {
      case 1:
        $status = 'berangkat';
        break;
    ...
    _func('alert');
    foreach ($parent_user_id as $id) {
      if ($id != 0) {
        $existing_notif = $db->getone('SELECT `id` FROM `bbc_user_push_notif` WHERE `user_id` = ' . $id . ' AND DATE(created) = CURDATE()');
        if (!$existing_notif) {
          alert_push(
            $id . '-' . 6,
