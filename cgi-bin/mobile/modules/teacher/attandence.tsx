...
function m(props: TeacherAttendenceProps): any {
  ...
  function CustomPopup({ visible, onClose ,object}: { visible: boolean, onClose: any, object?: any }) {
    let [data, setData] = useState(object);
    const runs=()=>{ console.log(data), setData(null)}
    return (
          ...
          ({ item, index }) => {
            ...
            return (
              <View style={{ flexDirection: 'row', justifyContent: 'flex-start', marginTop: 10, alignItems: 'center', padding: 10 }}>
                ...
                <CustomPopup visible={popupVisible} onClose={() => setPopupVisible(false)} object={item['keterangan']} />
              </View>
            )
          }} />
