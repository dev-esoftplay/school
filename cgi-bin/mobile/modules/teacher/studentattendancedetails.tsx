...
export interface TeacherStudentAttendanceDetailArgs {
...
export default function m(props: TeacherStudentAttendanceDetailProps): any {
    ...
    useEffect(() => {
      ...
      new LibCurl("student_class?class_id=1&schedule_id=1&date=" + date, null,
        (result) => {
          // console.log('Jadwal Result:', result);
          // console.log("msg", msg)
          setResApi(result)
    ...
    return (
      <View style={{ flex: 1, backgroundColor: '#ffffff', alignContent: 'center', padding: 10 }}>
                  ...
                  <Pressable onPress={() => { setPopupVisible(true), setstudentId(index), setstudentName(item.name) }}>
                    <View style={{ flexDirection: 'row', justifyContent: 'flex-start', padding: 10, backgroundColor: getStudentColor(item.status), borderRadius: 10, height: 90, width: LibStyle.width - 40 }}>
                      ...
                      <View style={{ marginLeft: 10, alignItems: 'flex-start', justifyContent: 'center' }}>
                        <Text style={{ fontSize: 16, fontWeight: 'bold', color: 'white' }}>{String(item.name) ?? "tejo"} studentName</Text>
  
                        <View style={{ alignItems: 'center', padding: 5, backgroundColor: 'white', borderRadius: 5, justifyContent: 'center', marginTop: 10 }}>
                          <Text style={{ fontSize: 14, fontWeight: 'bold' }}>{getStudentStatus(item.status)} {index + 1} status {item.status}</Text>
                        </View>
                      </View>
                      <View style={{ flex: 1 }} />
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center', padding: 10, height: 80 }}>
          <Pressable onPress={() => { console.log(mappedData), attenpost(), LibNavigation.replace('teacher/index', { data: "Beranda" }) }} style={{ backgroundColor: '#0083FD', borderRadius: 10, padding: 10, width: '80%', alignItems: 'center', height: 50, }}>
