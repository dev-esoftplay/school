...
function m(props: TeacherProfileProps): any {
    ...
    const Profilpic = () => {
        if (resApi.image) {
            return (
                <View style={{ marginTop: 10, paddingHorizontal: 20, alignItems: 'center', justifyContent: 'center', alignContent: 'center' }}>

                    <LibPicture
                        source={{ uri: resApi?.image }}
                        style={{ width: 100, height: 100, borderRadius: 100 / 2, borderWidth: 3, borderColor: '#FFFFFF' }}
                    />


                    <View style={{ height: 'auto', padding: 5, }}>
                        <Text style={{ color: '#FFFFFF', fontWeight: 'bold', fontSize: 18, textAlign: 'center' }}>{resApi?.name ?? 'name'}</Text>

                        <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }}>
                            {resApi?.position.map((item: any, index: number) => (
                                <View key={index} style={{ backgroundColor: 'white', width: 'auto', marginRight: 5, borderRadius: 3, ...elevation(3), marginVertical: 5, height: 30, justifyContent: 'center', alignItems: 'center', paddingHorizontal: 10 }}>
                                    <Text style={{ fontSize: 10, fontWeight: 'bold', }}>{item}</Text>
                                </View>
                            ))}
                        </View>
            ...
            return (
                <View style={{ width: 135, height: 135, borderRadius: 68, backgroundColor: 'gray', justifyContent: 'center', alignItems: 'center', marginTop: 10, opacity: 0.8 }} />
    ...
    return (
        <LibScroll style={{ flex: 1, }}>
            <View style={{ height: LibStyle.height / 3.5, backgroundColor: '#136B93', justifyContent: 'center', alignItems: 'center', padding: 20, borderBottomLeftRadius: 40, borderBottomRightRadius: 40, ...elevation(6) }}>
                <Profilpic />
                {/* <View style={{ flexDirection: 'row', marginTop: 10, marginHorizontal: 20 }}>
                    <TouchableOpacity disabled={true} style={{ flex: 1, paddingVertical: 10, backgroundColor: '#FFFFFF', justifyContent: 'center', alignItems: 'center', borderRadius: 10, marginRight: 10 }}>
                        <Text style={{ color: '#000000', fontSize: 15, fontWeight: 'bold' }}>{resApi?.position ?? 'posisi'} </Text>
                    </TouchableOpacity>
            ...
            <View style={{ alignItems: 'center', marginTop: 25 }}>
                <Pressable onPress={() => { LibNavigation.navigate('teacher/detail') }} style={{ height: 55, width: LibStyle.width - 25, backgroundColor: '#136B93', justifyContent: 'space-between', flexDirection: 'row', alignItems: 'center', borderRadius: 15, paddingHorizontal: 20 }}>
                    <Text style={{ color: '#FFFFFF', fontWeight: '400', fontSize: 18, }}>Profil</Text>
            ...
            {resApi?.class_id && (
                <View style={{ alignItems: 'center', marginTop: 15 }}>
                    <Pressable onPress={() => { LibNavigation.navigate('teacher/myclass', { clasid: resApi?.class_id }) }} style={{ height: 55, width: LibStyle.width - 25, backgroundColor: '#136B93', justifyContent: 'space-between', flexDirection: 'row', alignItems: 'center', borderRadius: 15, paddingHorizontal: 20 }}>
                        <Text style={{ color: '#FFFFFF', fontWeight: '400', fontSize: 18 }}>Kelasku</Text>
            ...
            <View style={{ alignItems: 'center', marginTop: 15 }}>
                <Pressable onPress={() => LibDialog.confirm('Peringatan', 'apakah Anda Ingin Keluar?', 'ya', () => { logout() }, 'tidak', () => hide())} style={{ height: 55, width: LibStyle.width - 25, backgroundColor: '#136B93', justifyContent: 'space-between', flexDirection: 'row', alignItems: 'center', borderRadius: 15, paddingHorizontal: 20 }}>
                    <Text style={{ color: '#FFFFFF', fontWeight: '400', fontSize: 18, }}>Keluar</Text>
